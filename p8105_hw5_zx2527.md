...
monotonically with the true value of mu. When mu=0 (when the null
hypothesis is true), the probability of rejecting the null is
approximately 0.05, which matches the nominal type I error. As mu
increases from 0 to 6, the probability of rejecting null hypothesis
increases. This pattern is expected because a larger effect size
produces a larger t-statistic on average, making the test more sensitive
and more likely to detect the deviation from the null.
...
ncols <- ncol(homicide_df)
```

Description for the raw data:

This dataset includes 52179 individuals and 13 variables. It includes
